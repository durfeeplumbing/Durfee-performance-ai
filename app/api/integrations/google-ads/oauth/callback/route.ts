...
export async function GET(request:Request){
  const user=await getCurrentUser();if(!user)return NextResponse.redirect(`${marketingBaseUrl()}/login`);if(user.role!=='owner')return NextResponse.redirect(`${marketingBaseUrl()}/marketing/connections?provider=google_ads&status=forbidden`);
  ...
  if(denied)return NextResponse.redirect(`${marketingBaseUrl()}/marketing/connections?provider=google_ads&status=denied`);
  try{
    ...
    return NextResponse.redirect(`${marketingBaseUrl()}/marketing/connections?provider=google_ads&status=authorized`);
  }catch(error){
    console.error('Google Ads OAuth callback failed',error instanceof Error?error.message:error);return NextResponse.redirect(`${marketingBaseUrl()}/marketing/connections?provider=google_ads&status=error`);
  }
}
