...
export async function GET(request:Request){
  const user=await getCurrentUser();if(!user)return NextResponse.redirect(`${marketingBaseUrl()}/login`);if(user.role!=='owner')return NextResponse.redirect(`${marketingBaseUrl()}/marketing/providers?provider=meta_ads&status=forbidden`);
  ...
  if(denied)return NextResponse.redirect(`${marketingBaseUrl()}/marketing/providers?provider=meta_ads&status=denied`);
  try{
