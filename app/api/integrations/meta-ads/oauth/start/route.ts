...
export async function GET(){
  ...
  const appId=process.env.META_ADS_APP_ID?.trim();if(!appId)return NextResponse.redirect(`${marketingBaseUrl()}/marketing/connections?provider=meta_ads&status=configuration_required`);
  const redirectUri=`${marketingBaseUrl()}/api/integrations/meta-ads/oauth/callback`;
  const version=(process.env.META_GRAPH_VERSION||'v24.0').trim();
