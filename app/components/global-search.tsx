...
export default function GlobalSearch(){
  ...
  return <div ref={boxRef} style={{position:'relative',width:'min(760px,100%)'}}>
    <div style={{display:'flex',alignItems:'center',gap:10,background:'#fff',border:'1px solid #d7dce2',borderRadius:12,padding:'0 12px',boxShadow:'0 2px 8px rgba(15,23,42,.05)'}}>
      ...
      <input ref={inputRef} value={query} onChange={e=>setQuery(e.target.value)} onFocus={()=>setOpen(true)} placeholder="Search customers, jobs, equipment, memberships, estimates, invoices, parts…" aria-label="Search Durfee Performance AI" style={{width:'100%',border:0,outline:'none',padding:'12px 0',fontSize:15,background:'transparent'}}/>
    ...
    {open&&query.trim().length>=2&&<div style={{position:'absolute',zIndex:1000,top:'calc(100% + 8px)',left:0,right:0,maxHeight:'70vh',overflowY:'auto',background:'#fff',border:'1px solid #d7dce2',borderRadius:14,boxShadow:'0 18px 50px rgba(15,23,42,.18)',padding:8}}>
      ...
      {!loading&&!results.length&&<div style={{padding:18}}><b>No matches found</b><div style={{marginTop:4,color:'#64748b',fontSize:13}}>Try a customer, phone, address, equipment model/serial, membership, job, estimate, invoice, SKU, PO, employee, call note, or review.</div></div>}
