...
export default async function ZeroInvoiceApprovalPage(){
  ...
  const allowed=managerRole&&((await hasPermission('manage_jobs'))||(await hasPermission('manage_billing')));
  if(!allowed) return <main style={{fontFamily:'system-ui',maxWidth:900,margin:'auto',padding:32}}><h1>Zero-Invoice Approvals</h1><p>Owner or manager authorization is required.</p></main>;
  ...
  return <main style={{fontFamily:'system-ui',maxWidth:1000,margin:'auto',padding:32}}>
    ...
    <p>Technicians cannot close a job with a $0 invoice until an owner or manager authorizes it here. Approval is single-use and is consumed when the job closes.</p>
    ...
    <section style={{display:'grid',gap:16,marginTop:24}}>{rows.map((r:any)=><article key={r.jobId} style={{border:'1px solid #ddd',borderRadius:16,padding:18}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10}}>
        ...
        <div>Requested by<br/><b>{r.requestedBy||'Employee'}</b><br/><small>{r.requestedAt?new Date(r.requestedAt).toLocaleString('en-US',{timeZone:'America/New_York'}):'—'}</small></div>
      </div>
      ...
      <form action={approveZeroInvoiceCloseout} style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <button type="submit">Authorize $0 Closeout</button>
      </form>
