...
export default async function RootLayout({children}:{children:ReactNode}){
  ...
  const visibleNav=user.role==='owner'?[...nav,['Permissions','/settings/permissions']]:nav;
  return <html lang="en"><body><div className="shell"><aside className="sidebar"><div className="brand">DURFEE<br/><span>PERFORMANCE AI</span></div><nav>{visibleNav.map(([label,href])=><Link key={href} href={href}>{label}</Link>)}</nav><Link className="fieldLink" href="/field">Open Field App</Link><div style={{marginTop:20,fontSize:12,opacity:.8}}>{user.name}<br/>{user.role}</div><form action={logout}><button type="submit" style={{marginTop:10}}>Sign out</button></form></aside><div className="content">{children}</div></div></body></html>
}
