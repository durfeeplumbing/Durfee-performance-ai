...
export async function syncServiceTitanReferenceData() {
  ...
  await saveResource(supabase, 'technicians', technicians, info);
  await saveResource(supabase, 'business_units', businessUnits, info);
...
export async function syncServiceTitanCrmData() {
  ...
  const customers = await serviceTitanGetAll('/crm/v2/tenant/{tenant}/customers');
  await saveResource(supabase, 'customers', customers, info);

  const locations = await serviceTitanGetAll('/crm/v2/tenant/{tenant}/locations');
  await saveResource(supabase, 'locations', locations, info);
