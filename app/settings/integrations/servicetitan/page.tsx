...
export default async function ServiceTitanIntegrationPage() {
  ...
  const totalCached = resources.reduce((sum, resource) => sum + Number(countByResource[resource] ?? 0), 0);
  const healthy = configured && loadedResources === resources.length && running.length === 0 && !state?.last_error;
  ...
  return <main>
    ...
    <section className="card" style={{margin:'20px 0',padding:20,border:`1px solid ${healthColor}`,background:healthBg}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'center',flexWrap:'wrap'}}>
        <div>
          <div style={{fontSize:13,fontWeight:700,textTransform:'uppercase',letterSpacing:'.05em'}}>System health</div>
          <div style={{fontSize:28,fontWeight:800,color:healthColor}}>{healthy ? '✓ ' : '● '}{healthLabel}</div>
        </div>
      ...
      <div style={{marginTop:14,fontSize:14}}>
        ...
        {state?.last_error ? <span style={{color:'#b42318'}}>Latest integration error: {state.last_error}</span> : <span style={{color:healthColor}}>No active integration error.</span>}
