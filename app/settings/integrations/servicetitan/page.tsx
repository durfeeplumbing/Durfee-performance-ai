...
import { syncServiceTitanCrmData, syncServiceTitanJobsData, syncServiceTitanReferenceData } from './actions';
...
export default async function ServiceTitanIntegrationPage() {
  ...
  const resources = ['technicians','business_units','customers','locations','jobs','appointments'] as const;
  const [stateResult, runsResult, ...counts] = await Promise.all([
    supabase.from('service_titan_integration_state').select('*').maybeSingle(),
    supabase.from('service_titan_sync_runs').select('id,resource,status,records_seen,records_upserted,error,started_at,completed_at').order('started_at', { ascending: false }).limit(20),
