...
begin
  ...
  if p_resource not in ('technicians','business_units','customers','locations','jobs','appointments','invoices','payments','memberships','pricebook_categories','pricebook_services','pricebook_materials','pricebook_equipment') then raise exception 'Unsupported ServiceTitan resource'; end if;
  ...
  values (true,p_environment,p_tenant_id,now(),null,now()) on conflict (singleton) do update set environment=excluded.environment,tenant_id=excluded.tenant_id,last_successful_sync=excluded.last_successful_sync,last_error=null,updated_at=now();
